// Import all graphic design articles with FULL HTML rich text and SEO preservation
...
// Article mapping with facets - All graphic design articles
// Note: Each article gets BOTH the parent 'graphic_design' facet AND its specific subcategory
const articleMappings = [
    ...
    {
        filename: 'illustration.html',
        facets: ['graphic_design', 'illustration', 'beginner', 'step_by_step_guide', 'designer']
    },
    {
        filename: 'infographic.html',
        facets: ['graphic_design', 'infographic', 'beginner', 'step_by_step_guide', 'designer']
    },
    {
        filename: 'logo-design.html',
        facets: ['graphic_design', 'logo_design', 'beginner', 'step_by_step_guide', 'designer']
    },
    {
        filename: 'web-design.html',
        facets: ['graphic_design', 'web_design', 'beginner', 'step_by_step_guide', 'designer']
    },
    {
        filename: 'product-design.html',
        facets: ['graphic_design', 'product_design', 'intermediate', 'step_by_step_guide', 'designer']
    },
    {
        filename: 'mobile-app-design.html',
        facets: ['graphic_design', 'mobile_app_design', 'intermediate', 'step_by_step_guide', 'designer']
    },
    {
        filename: 'wrap-design.html',
        facets: ['graphic_design', 'wrap_design', 'intermediate', 'step_by_step_guide', 'designer']
    }
];

async function importGraphicDesignArticles() {
    try {
        console.log('🎨 Importing Graphic Design articles with RICH TEXT and SEO preservation...\n');
        ...
        const tempDir = path.join(__dirname, '..', 'temp', 'graphic');
        let imported = 0;
        let updated = 0;
        ...
        const neededFacetValues = [
            'graphic_design', 'illustration', 'infographic', 'logo_design', 
            'web_design', 'product_design', 'mobile_app_design', 'wrap_design'
        ];

        for (const value of neededFacetValues) {
            const exists = await FacetValue.findOne({ where: { value } });
            if (!exists) {
                console.log(`  ⚠️  Missing facet value: ${value} - You may need to add it to seed-facets.js`);
            }
        }
        console.log('');
        ...
        // Print summary
        console.log('═══════════════════════════════════════════════════════════════');
        console.log('🎉 GRAPHIC DESIGN ARTICLES IMPORT COMPLETED!');
        console.log('═══════════════════════════════════════════════════════════════');
        ...
        if (articleDetails.length > 0) {
            ...
            articleDetails.forEach((detail, index) => {
                ...
                console.log(`   ID: ${detail.id} | Public ID: ${detail.public_id}`);
        ...
        console.log('✅ All HTML content preserved (lists, links, bold, structure)');
...
if (require.main === module) {
    importGraphicDesignArticles();
}

module.exports = importGraphicDesignArticles;
